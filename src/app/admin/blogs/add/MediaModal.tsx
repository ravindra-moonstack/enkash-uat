...
export default function MediaModal({ onClose, onSelect, title = "Media Library", defaultType = "all" }: MediaModalProps) {
    ...
    return (
        <div className={styles.modalOverlay} onClick={onClose} style={{ zIndex: 9999 }}>
            <div className={styles.modalContent} onClick={e => e.stopPropagation()} style={{ width: '95%', maxWidth: '1400px', height: '94vh', display: 'flex', flexDirection: 'column' }}>
                ...
                <div className={styles.tabBar} style={{ padding: '0 20px', borderBottom: '1px solid #ddd', display: 'flex', gap: '20px' }}>
                    <button
                        className={`${styles.tab} ${activeTab === 'upload' ? styles.active : ''}`}
                        onClick={() => setActiveTab('upload')}
                        style={{ padding: '15px 5px', background: 'none', border: 'none', borderBottom: activeTab === 'upload' ? '3px solid #2271b1' : '3px solid transparent', cursor: 'pointer', fontWeight: 600, color: activeTab === 'upload' ? '#2271b1' : '#646970' }}
                    >
                        Upload files
                    </button>
                    <button
                        ...
                        style={{ padding: '15px 5px', background: 'none', border: 'none', borderBottom: activeTab === 'library' ? '3px solid #2271b1' : '3px solid transparent', cursor: 'pointer', fontWeight: 600, color: activeTab === 'library' ? '#2271b1' : '#646970' }}
                    >
                        Media Library
                    </button>
                </div>
                ...
                {activeTab === "upload" ? (
                    <div className={styles.modalBody} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff' }}>
                        <div className={styles.uploadBox} style={{ border: '2px dashed #c3c4c7', padding: '60px', textAlign: 'center', background: '#f0f0f1' }}>
                            ...
                            <button className={styles.selectFilesBtn} onClick={handleUploadClick} disabled={uploading} style={{ padding: '8px 16px', border: '1px solid #2271b1', color: '#2271b1', background: '#f6f7f7', borderRadius: '4px', cursor: 'pointer' }}>
                                {uploading ? "Uploading..." : "Select Files"}
                            </button>
                            <p style={{ marginTop: '20px', fontSize: '12px', color: '#50575e' }}>Maximum upload file size: 512 MB.</p>
                        </div>
                    </div>
                ) : (
                    <>
                        <div className={styles.filterBar} style={{ padding: '10px 20px', background: '#fff', borderBottom: '1px solid #ddd' }}>
                            <div className={styles.filterLeft}>
                                <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} style={{ padding: '4px 8px', borderRadius: '4px', border: '1px solid #8c8f94', marginRight: '10px' }}>
                            ...
                            <div className={styles.filterRight}>
                                <input
                                    ...
                                    style={{ padding: '4px 8px', borderRadius: '4px', border: '1px solid #8c8f94' }}
                                />
                            </div>
                        </div>

                        <div className={styles.modalBody} style={{ flex: 1, overflow: 'hidden', display: 'flex', gap: 0 }}>
                            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
                                <div className={styles.mediaGrid} style={{
                                    ...
                                    {loading && mediaItems.length === 0 ? (
                                        <div style={{ textAlign: 'center', padding: '50px', gridColumn: '1 / -1' }}>Loading...</div>
                                    ) : (
                                        mediaItems.map((item) => (
                                            <div
                                                key={item.id}
                                                ...
                                                style={{
                                                    aspectRatio: '1',
                                                    border: selectedMedia?.id === item.id ? '4px solid #2271b1' : '1px solid #ddd',
                                                    position: 'relative',
                                                    cursor: 'pointer',
                                                    overflow: 'hidden'
                                                }}
                                            >
                                                {selectedMedia?.id === item.id && (
                                                    <div className={styles.checkIndicator} style={{ position: 'absolute', top: 5, right: 5, background: '#2271b1', color: '#fff', borderRadius: '2px', padding: '2px', zIndex: 1 }}><i className="bi bi-check"></i></div>
                                                )}
                                                {(() => {
                                                    const isImg = item.image_url && item.image_url.match(/\.(jpeg|jpg|gif|png|webp|svg)$/i);
                                                    return isImg ? (
                                                        <img src={getFileUrl(item.image_url)} alt={item.attachment_image_alt || "Media Item"} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                                    ) : (
                                                        <div className={styles.docIcon} style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#fff' }}>
                                                            <i className="bi bi-file-earmark-text" style={{ fontSize: '32px' }}></i>
                                                            <div style={{ fontSize: '10px', marginTop: '5px', wordBreak: 'break-all', textAlign: 'center', padding: '0 5px' }}>{item.image_url.split('/').pop()}</div>
                                                        </div>
                                                    )
                                                })()}
                                            </div>
                                ...
                                {mediaItems.length > 0 && page * itemsPerPage < totalItems && (
                                    <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                        <button
                                            onClick={loadMore}
                                            disabled={loading}
                                            style={{ padding: '8px 16px', background: '#2271b1', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                                        >
                            ...
                            {selectedMedia && (
                                <div className={styles.mediaDetails} style={{ width: '350px', background: '#f6f7f7', padding: '20px', borderLeft: '1px solid #ddd', overflowY: 'auto' }}>
                                    <h4 style={{ margin: '0 0 15px 0' }}>ATTACHMENT DETAILS</h4>
                                    <div style={{ display: 'flex', gap: '15px', marginBottom: '20px' }}>
                                        <div style={{ width: '80px', height: '80px' }}>
                                            {(() => {
                                                const isImg = selectedMedia.image_url && selectedMedia.image_url.match(/\.(jpeg|jpg|gif|png|webp|svg)$/i);
                                                return isImg ? <img src={getFileUrl(selectedMedia.image_url)} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <i className="bi bi-file-earmark-text" style={{ fontSize: '40px' }}></i>;
                                            })()}
                                        </div>
                                    ...
                                    <div className={styles.inputGroup} style={{ marginBottom: '10px' }}>
                                        <label style={{ display: 'block', fontSize: '12px', color: '#646970', marginBottom: '5px', textAlign: 'left' }}>Title</label>
                                        <input
                                            type="text"
                                    ...
                                    <div style={{ background: '#fff', borderTop: '1px solid #ddd', padding: '15px', margin: '0 -20px -20px -20px', position: 'sticky', bottom: 0 }}>
                                        <button
                                            onClick={() => onSelect({
                                            ...
                                            style={{ width: '100%', padding: '10px', background: '#2271b1', color: '#fff', border: 'none', borderRadius: '3px', cursor: 'pointer', fontWeight: 600 }}
                                        >
                                            Select
                                        </button>
                                    </div>
