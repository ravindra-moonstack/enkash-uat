...
const MediaCoveragePage = () => {
    ...
    const fetchMedia = async (page: number) => {
        setLoading(true)
        try {
            const res = await fetch(`/api/resources/media-coverage?page=${page}&limit=6`)
                ...
                <div className="max-w-auto">
                    <div className={styles.banner_row}>
                        <div className={styles.banner_content}>
                            <DynamicHeading content={[{ title: "EnKash secures PPI license; aims to lead in Corporate Cards and Spend Management", className: styles.banner_title, color: "color-white f-7" }]} />
                            <CommonButton title="Read More" width={"fit-content"} url={"https://www.business-standard.com/content/press-releases-ani/enkash-secures-prestigious-ppi-license-strengthening-its-leadership-in-corporate-cards-and-spend-management-125040900400_1.html"} />
                        </div>
                        <div className={styles.banner_image_wrapper}>
                            <Image
                                src="/uploads/2026/02/tablet_hand.png"
                                alt="Media Coverage"
                                width={500}
                                height={600}
                    ...
                    <div className={styles.latest_news_wrapper}>
                        ...
                        <div className={styles.news_grid}>
                            {loading ? (
                                ...
                                [...Array(6)].map((_, i) => (
                                    <div key={i} className={styles.news_card} style={{ height: '450px', backgroundColor: '#eee', opacity: 0.5 }}></div>
                                ))
                            ) : (
                                data.map((item) => (
                                    <div key={item.id} className={styles.news_card}>
                                        ...
                                        <div className={styles.card_content}>
                                            <Link href={extractHref(item.media_coverage_media_link)} target="_blank">
                                                <div className={styles.title_wrapper}><h3>{item.media_coverage_heading || item.title}</h3></div>
                                            </Link>
                                            <div className={styles.card_footer}>
                                                <span className={styles.date}>{item.media_coverage_date}</span>
                                                <CommonButton title="Read More" theme="blue" className={styles.read_more_btn} url={extractHref(item.media_coverage_media_link)} arrowType="ios" arrow />
                                            </div>
                                        </div>
                                    </div>
                                ))
                            ...
                            <div className={styles.pagination}>
                                ...
                                {[...Array(pagination.totalPages)].map((_, i) => {
                                    ...
                                    if (p === 1 || p === pagination.totalPages || (p >= pagination.page - 1 && p <= pagination.page + 1)) {
                                        ...
                                        return <span key={p} className={`${styles.page_btn} styles.dots`}>...</span>
                                    }
                                    return null;
                                })}
