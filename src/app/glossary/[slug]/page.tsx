...
async function getTerm(slug: string) {
    ...
    const reservedSlugs = ['favicon.ico', 'robots.txt', 'sitemap.xml', 'api', '_next'];
...
async function getTerms(letter: string) {
    try {
        const baseUrl = getApiBaseUrl()
        const res = await fetch(`${baseUrl}/api/glossary/letter/${encodeURIComponent(letter)}`, { cache: 'no-store' })
        if (!res.ok) return []
        return res.json()
    } catch (error) {
