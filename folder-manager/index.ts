...
async function cleanEmptyParents(fullPath: string, rootDir: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    console.log("\x1b[33m[Dry Run] Preview mode — no files will be moved.\x1b[0m")
  }
  ...
  // ── LIST ──────────────────────────────────────────────────────────────────────
  if (action === "list") {
    let used = 0, unused = 0, untracked = 0, totalSize = 0
    ...
    console.log(`  Total files   : ${uploadFiles.length}  (${formatBytes(totalSize)})`)
    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      for (const file of batch) {
        ...
        try {
          ...
          console.error(`  \x1b[31m[Error] ${file.path}: ${err instanceof Error ? err.message : String(err)}\x1b[0m`)
          skipped++
        }
      }
  ...
  if (action === "backup") {
    ...
    await moveFiles(candidates, UPLOADS_DIR, BACKUP_DIR, `Backup (type=${type})`)
  }
