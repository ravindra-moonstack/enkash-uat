...
test.describe("Core Company Pages", () => {
  ...
  test.describe("Careers (/careers)", () => {
    test("CAR-01: View Open Roles functionality", async ({ page }) => {
      ...
      const openRolesBtn = page.getByRole("link", { name: /View Open Roles/i }).or(page.getByRole("button", { name: /View Open Roles/i }))
  ...
  test.describe("Corporate Information & Policies", () => {
    test("CORP-01 & CORP-02: Corporate Info renders text blocks", async ({ page }) => {
    ...
    test("CORP-01 & CORP-02: Policies renders text blocks", async ({ page }) => {
