...
test.describe("Admin & Authentication", () => {
  ...
  test.describe("Admin Dashboard Access (/admin/dashboard)", () => {
    test("ADM-01: Redirects to login if unauthenticated", async ({ page }) => {
      ...
      const isLoginOrRedirected = page.url().includes("login") || await page.locator("form").count() > 0
      expect(isLoginOrRedirected).toBeTruthy()
    })
    
    test("ADM-03: Quill Editor component renders (mocking session)", async ({ page }) => {
