//Home Page  Aniamtion
export const phoneAndTabAnimation = () =>
  import("../../app/img/lottie/group.json")
export const payablesAnimation = () =>
  import("../../app/img/lottie/payables.json")
export const recieveableAnimation = () =>
  import("../../app/img/lottie/recievables.json")
export const expenseManagementAnimation = () =>
  import("../../app/img/lottie/expense-management.json")
export const corporateCardAnimation = () =>
  import("../../app/img/lottie/credit-card.json")
export const diyCardAnimation = () => import("../../app/img/lottie/diy.json")
export const paymentGatewayHomepageAnimation = () =>
  import("../../app/img/lottie/payment-gateway.json")

//corporate-cards Animation
export const freedomBannerAnimation = () =>
  import("../../app/cards/img/lottie/banner.json")
export const travelCardAnimation = () =>
  import("../../app/cards/img/lottie/travel.json")
export const saasAnimation = () =>
  import("../../app/cards/img/lottie/saas.json")
export const virtualCardAnimation = () =>
  import("../../app/cards/img/lottie/virtual.json")
export const digitalAnimation = () =>
  import("../../app/cards/img/lottie/digital-marketing.json")
export const mealCardAnimation = () =>
  import("../../app/cards/img/lottie/meal.json")

export const fuelAnimation = () =>
  import("../../app/cards/img/lottie/fuel.json")

export const purchaseCardAnimation = () =>
  import("../../app/cards/img/lottie/purchase.json")
export const virtualAnimation = () =>
  import("../../app/cards/img/lottie/virtual-card.json")

//Recievables Animation
export const paymentGatewayAnimation = () =>
  import("../../app/collect-payments/img/lottie/payment-gateway.json")
export const recievablesBannerAnimation = () =>
  import("../../app/collect-payments/img/lottie/banner.json")
export const invoiceAnimation = () =>
  import("../../app/collect-payments/img/lottie/invoice.json")
export const bulkCollectAnimation = () =>
  import("../../app/collect-payments/img/lottie/bulk-collect.json")
export const paymentLinkAnimation = () =>
  import("../../app/collect-payments/img/lottie/payment-link.json")
export const quickCollectAnimation = () =>
  import("../../app/collect-payments/img/lottie/quick-collect.json")
export const virtualAccountAnimation = () =>
  import("../../app/collect-payments/img/lottie/virtual.json")

//expense-management Animation
export const ofexBannerAnimation = () =>
  import("../../app/expense-management/img/lottie/banner.json")
export const heirarchyControlAnimation = () =>
  import("../../app/expense-management/img/lottie/hierarchy.json")
export const budgetAndAdvanceAnimation = () =>
  import("../../app/expense-management/img/lottie/budget-and-advances.json")
export const reimbursementAnimation = () =>
  import("../../app/expense-management/img/lottie/reimbursement.json")
export const scanAndDropAnimation = () =>
  import("../../app/expense-management/img/lottie/scan-and-drop.json")
export const policyAnimation = () =>
  import("../../app/expense-management/img/lottie/policy.json")

//Loyalty Animation
export const loyaltyBannerAnimation = () =>
  import("../../app/rewards/img/lottie/banner.json")
export const loyaltyOfferAnimation = () =>
  import("../../app/rewards/img/lottie/offer.json")
export const employeeeRewardAnimation = () =>
  import("../../app/rewards/img/lottie/employee-reward.json")
export const offerAnimation = () =>
  import("../../app/rewards/img/lottie/offer.json")
export const giftCardAnimation = () =>
  import("../../app/rewards/img/lottie/gift-card.json")
export const channelIncentiveAnimation = () =>
  import("../../app/rewards/img/lottie/channel-incentive.json")
export const brandVoucherAnimation = () =>
  import("../../app/rewards/img/lottie/brand-voucher.json")

//Olympus Animation
export const olympusBannerAnimation = () =>
  import("../../app/olympus/img/lottie/banner.json")
export const olympusVendorAnimation = () =>
  import("../../app/olympus/img/lottie/vendor.json")
export const olympusGSTAnimation = () =>
  import("../../app/olympus/img/lottie/gst.json")
export const olympusUtilityAnimation = () =>
  import("../../app/olympus/img/lottie/utility.json")
export const olympusRentalAnimation = () =>
  import("../../app/olympus/img/lottie/rental.json")
export const olympusMultiBankAnimation = () =>
  import("../../app/olympus/img/lottie/multi-bank.json")
export const olympusCollectAnimation = () =>
  import("../../app/olympus/img/lottie/collect.json")
export const olympusPaymentAnimation = () =>
  import("../../app/olympus/img/lottie/payment-links.json")
export const olympusBulkCollectAnimation = () =>
  import("../../app/olympus/img/lottie/bulk-collect.json")
export const olympusInvoicesAnimation = () =>
  import("../../app/olympus/img/lottie/invoices.json")

//Loyalty Section Homepage
export const loyaltyOne = () => import("../../app/img/lottie/loyalty-1.json")
export const loyaltyTwo = () => import("../../app/img/lottie/loyalty-2.json")
export const loyaltyThree = () => import("../../app/img/lottie/loyalty-3.json")
export const loyaltyFour = () => import("../../app/img/lottie/loyalty-4.json")
export const loyaltyFive = () => import("../../app/img/lottie/loyalty-5.json")

//Dashbaord Section Homepage
export const manage = () => import("../../app/img/lottie/manage.json")
export const track = () => import("../../app/img/lottie/track.json")
export const optimize = () => import("../../app/img/lottie/optimize.json")
export const automate = () => import("../../app/img/lottie/automate.json")

//Payables Animation
export const payablesBannerAnimation = () =>
  import("../../app/make-payments/img/lottie/banner.json")
export const payablesTaxAnimation = () =>
  import("../../app/make-payments/img/lottie/tax-pay.json")
export const payablesVendorAnimation = () =>
  import("../../app/make-payments/img/lottie/vendor-pay.json")
export const payablesRentalAnimation = () =>
  import("../../app/make-payments/img/lottie/rental-pay.json")
export const payablesBulkAnimation = () =>
  import("../../app/make-payments/img/lottie/bulk-pay.json")
export const payablesBillAnimation = () =>
  import("../../app/make-payments/img/lottie/bill-pay.json")


//Partner Animation

