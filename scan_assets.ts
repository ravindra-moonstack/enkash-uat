...
async function main() {
  console.log("\x1b[35m=== Uploads & Database Reference Asset Scanner ===\x1b[0m");
  ...
  if (process.env.DB_HOST && process.env.DB_NAME) {
    console.log(`\x1b[34m[1/3] Connecting to database: ${process.env.DB_HOST} (${process.env.DB_NAME})...\x1b[0m`);
    ...
    try {
      ...
      console.log(`Loaded ${dbPostsContent.length} post content records from posts table.`);
      isDbConnected = true;

    } catch (err: any) {
      console.error(`\x1b[31m[Warning] Database connection/query failed: ${err.message}\x1b[0m`);
    ...
    console.log(`\x1b[33m[Warning] DB_HOST or DB_NAME not configured in env. Skipping database check.\x1b[0m`);
  }
  ...
  console.log(`\x1b[34m[2/3] Scanning codebase files inside: ${CODE_DIRS.join(", ")}...\x1b[0m`);
  ...
  console.log(`\x1b[34m[3/3] Scanning physical files in ${UPLOADS_DIR}...\x1b[0m`);
